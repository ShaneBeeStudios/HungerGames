...
//			try {
//				Util.sendMini(sender, lang.cmd_delete_attempt.replace("<arena>", gameArenaData.getName()));
//
//				if (gameArenaData.getStatus() == Status.BEGINNING || gameArenaData.getStatus() == Status.RUNNING) {
//					Util.sendMini(sender, "  &7- &cGame running! &aStopping..");
//					g.getGameBlockData().forceRollback();
//					g.stop(false);
//				}
//				if (!gamePlayerData.getPlayers().isEmpty()) {
//					Util.sendMini(sender, lang.cmd_delete_kicking);
...
//				Util.sendMini(sender, lang.cmd_delete_deleted.replace("<arena>", gameArenaData.getName()));
//				plugin.getGames().remove(g);
//			} catch (Exception e) {
//				Util.sendMini(sender, lang.cmd_delete_failed);
//			}
//		} else {
//			Util.sendMini(sender, lang.cmd_delete_noexist);
//		}
		return true;
	}
}
